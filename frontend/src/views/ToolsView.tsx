 ...
 *
 * CRITICAL RULE: If Con_Restricted is added, it is ALWAYS placed at line 0!
 */
function mergeCfgContent(
  ...
  // If Con_Restricted needs to be added newly, prepend to the very top (MUSS ganz oben stehen!)
...
export const ToolsView: React.FC = () => {
  // Navigation Tabs:
  // 'maintenance': System-Wartung, Shaders, Hardware-Benchmark
  // 'cfg': user.cfg Vollbild-Live-Editor
  // 'tuning': Strukturierte visuelle Regler & Schalter mit Live-Merge
  // 'reference': 2026 Befehls-Lexikon mit Erklärungen & 1-Klick Anwenden
  // 'backups': user.cfg Backup-Tresor mit Snapshot-Historie & 1-Klick Rollback
  // 'keybinds': Steuerungs-Tresor & Cloud-Sync
  const [activeTab, setActiveTab] = useState<'maintenance' | 'cfg' | 'tuning' | 'reference' | 'backups' | 'keybinds'>('cfg');
  ...
  // Comprehensive Tuning Controls (Parsed dynamically from user.cfg)
  ...
  const applyPreset = (type: '5800x3d_5070' | 'esport' | 'quality' | 'minimal', replaceAll: boolean = false) => {
    if (type === '5800x3d_5070') {
      ...
      showToast('⭐ Profil "AMD 5800X3D & RTX 5070" erfolgreich in bestehende Datei eingefügt (Merge)');
      return;
    }
  ...
  const isUserCfgSubTab = activeTab === 'cfg' || activeTab === 'tuning' || activeTab === 'reference' || activeTab === 'backups';
    ...
    <div className="p-6 space-y-6 max-w-7xl mx-auto">
      ...
      <div className="flex flex-wrap items-center gap-2 border-b border-slate-800 pb-3">
        ...
        >
          ...
          <span>2. user.cfg Live-Editor</span>
        </button>
        ...
        >
          ...
          <span>3. Grafik- &amp; Performance-Tuning</span>
        </button>
        ...
        >
          ...
          <span>4. Befehls-Lexikon (2026)</span>
        </button>
        ...
        >
          ...
          <span>5. Backup-Tresor &amp; Rollback</span>
        ...
        >
          ...
          <span>6. Steuerungs-Tresor (Keybinds)</span>
          {status?.keybindItems && status.keybindItems.length > 0 && (
            <span className="px-1.5 py-0.2 rounded-full bg-indigo-950 text-indigo-300 text-[10px] font-mono border border-indigo-800 font-bold">
      ...
      {/* Persistent user.cfg Suite Sub-Navigation Bar */}
      {isUserCfgSubTab && (
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 p-2 rounded-2xl bg-slate-900/80 border border-slate-800">
          <div className="flex flex-wrap items-center gap-1.5">
            ...
            >
              ...
              <span>🎛️ Visuelle Tuning-Schalter</span>
            </button>
            ...
            >
              ...
              <span>📖 Befehls-Lexikon (2026)</span>
            </button>
      ...
      {/* ══════════════════════════════════════════════════════════════
          TAB 2: USER.CFG LIVE CODE-EDITOR (FULL WIDTH)
          ══════════════════════════════════════════════════════════════ */}
      {activeTab === 'cfg' && (
        <div className="space-y-5">
          ...
          <div className="p-4 rounded-2xl bg-gradient-to-r from-slate-900/90 via-slate-900/70 to-slate-950 border border-slate-800 flex flex-col lg:flex-row lg:items-center justify-between gap-4">
            ...
            <div className="flex flex-wrap items-center gap-2">
              {/* Torsten's Highlight Preset: 5800X3D & RTX 5070 */}
              <div className="flex items-center rounded-xl bg-gradient-to-r from-amber-500/20 to-sky-500/20 border border-amber-500/40 p-1 shadow-md shadow-amber-500/10">
                <button
                  ...
                  title="Wendet das 5800X3D & RTX 5070 Profil per Merge auf deine bestehende Datei an (Kommentare bleiben erhalten)"
                >
                ...
                <button
                  ...
                  title="Ersetzt den Inhalt komplett durch die saubere 5800X3D & RTX 5070 Vorlage mit allen Erklärungen"
                >
                  Reines Template
                </button>
              </div>
              ...
              <button
                ...
                title="60 FPS Cap, 4GB StreamPool, maximale Stabilität"
              >
                💻 Minimal / 60 FPS
              </button>
            </div>
          ...
          {/* Main Full-Width Live Code Editor Card */}
          <div className="p-5 rounded-2xl bg-slate-900/90 border border-slate-800 flex flex-col shadow-2xl space-y-4">
            ...
            {/* Code Textarea with generous height */}
            <div className="relative rounded-xl bg-slate-950 border border-slate-800 font-mono text-xs overflow-hidden flex shadow-inner">
              <textarea
                ...
                spellCheck={false}
                className="w-full min-h-[520px] p-4 bg-transparent text-sky-100 resize-y focus:outline-none font-mono text-xs leading-relaxed selection:bg-sky-800/50"
                placeholder="; Star Citizen user.cfg Konfiguration&#10;Con_Restricted = 0&#10;r_VSync = 0&#10;sys_maxfps = 160&#10;..."
              />
            </div>

            {/* Editor Footer Notice & Shortcuts */}
      ...
      {/* ══════════════════════════════════════════════════════════════
          TAB 3: GRAFIK- & PERFORMANCE-TUNING (VISUAL CARDS STUDIO)
          ══════════════════════════════════════════════════════════════ */}
      {activeTab === 'tuning' && (
        <div className="space-y-6">
          ...
          {/* Grid of Categorized Tuning Cards */}
            ...
            <div className="p-5 rounded-2xl bg-slate-900/80 border border-slate-800 space-y-4 flex flex-col justify-between">
              <div>
                <div className="flex items-center space-x-2.5 pb-3 border-b border-slate-800 text-amber-400 font-bold text-xs tracking-wider">
                  ...
                  <span>3. HDR EINSTELLUNGEN (HIGH DYNAMIC RANGE)</span>
                </div>
            ...
            <div className="p-5 rounded-2xl bg-slate-900/80 border border-slate-800 space-y-4 flex flex-col justify-between">
              <div>
                <div className="flex items-center space-x-2.5 pb-3 border-b border-slate-800 text-indigo-400 font-bold text-xs tracking-wider">
                  ...
                  <span>6. SPRACHE &amp; INTERFACE (LOKALISIERUNG)</span>
                </div>
      ...
      {/* ══════════════════════════════════════════════════════════════
          TAB 4: BEFEHLS-LEXIKON & REFERENZ (STAND 2026)
          ══════════════════════════════════════════════════════════════ */}
      {activeTab === 'reference' && (
        <div className="space-y-6">
          {/* Header */}
          <div className="p-6 rounded-2xl bg-gradient-to-r from-emerald-950/40 via-slate-900/80 to-sky-950/40 border border-emerald-800/40 shadow-xl space-y-4">
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
              <div className="flex items-center space-x-3.5">
                ...
                <div>
                  <h2 className="text-base font-bold text-white tracking-wide">
                    ÜBERSICHT ALLER RELEVANTEN STAR CITIZEN USER.CFG BEFEHLE (STAND 2026)
                  </h2>
                  <p className="text-xs text-slate-400 mt-1">
                    Komplette Referenz mit Erklärungen, optimalen Werten und 1-Klick-Übernahme in deine Live-Konfiguration.
                  </p>
                </div>
              </div>
              ...
              <div className="flex items-center space-x-2 shrink-0">
                <button
                  onClick={() => applyPreset('5800x3d_5070')}
      ...
      {/* ══════════════════════════════════════════════════════════════
          TAB 5: USER.CFG BACKUP-TRESOR & 1-KLICK ROLLBACK
          ══════════════════════════════════════════════════════════════ */}
      {activeTab === 'backups' && (
        <div className="space-y-6">
          {/* Header Card */}
      ...
      {/* ══════════════════════════════════════════════════════════════
          TAB 6: STEUERUNGS-TRESOR (KEYBINDS & CLOUD)
          ══════════════════════════════════════════════════════════════ */}
      {activeTab === 'keybinds' && (
        <div className="space-y-6">
