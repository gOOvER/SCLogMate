    ...
    <div className="flex flex-col min-h-full space-y-4 font-mono">
      ...
      <div className="sc-glass rounded-xl p-3 border border-slate-800/80 flex flex-wrap items-center justify-between gap-3 text-xs">
        ...
        <div className="flex items-center space-x-3 flex-1 max-w-md">
          <div className="relative flex-1">
            ...
            <input
              ...
              className="w-full bg-slate-950/80 border border-slate-800 rounded-lg pl-8 pr-3 py-1.5 text-slate-200 text-xs focus:outline-none focus:border-cyan-500 transition-colors"
            />
          </div>
        ...
        <div className="flex items-center space-x-2">
          ...
          <button
            onClick={() => handleOpenReportModal()}
            className="px-3 py-1.5 rounded-lg bg-rose-600/80 hover:bg-rose-500 border border-rose-500 text-white font-bold transition-colors flex items-center space-x-1.5 shadow-lg shadow-rose-950/40"
          >
      ...
      <div className="sc-glass rounded-xl border border-slate-800 flex-1 flex flex-col min-h-[420px] max-h-[64vh] overflow-hidden">
        {/* Table Header */}
        <div className="grid grid-cols-12 gap-2 px-4 py-2.5 bg-slate-950/80 border-b border-slate-800 text-[11px] font-bold text-slate-400 uppercase tracking-wider shrink-0">
          <div className="col-span-2 md:col-span-1">Zeit</div>
          <div className="col-span-2 md:col-span-1">Kanal</div>
          <div className="col-span-3 md:col-span-2">Sender (Pilot)</div>
          <div className="col-span-4 md:col-span-7">Nachricht</div>
          ...
          {loading && messages.length === 0 ? (
            ...
            filteredMessages.map((msg) => {
              ...
              return (
                <div
                  key={msg.id}
                  className={`grid grid-cols-12 gap-2 px-3 py-2 rounded-lg text-xs items-center transition-colors group ${
                    msg.isFlagged
                ...
                >
                  {/* Timestamp */}
                  <div
                    className="col-span-2 md:col-span-1 text-[11px] text-slate-500 font-mono"
                    title={msg.timestamp}
                  >
                    {timeFormatted}
                  </div>
                  ...
                  <div className="col-span-2 md:col-span-1">
                    <span
                      className={`inline-block px-1.5 py-0.5 rounded text-[10px] font-bold border ${channelBadgeClass}`}
                    >
                      [{msg.channel}]
                    </span>
                  </div>
                  ...
                  {/* Sender */}
                  <div className="col-span-3 md:col-span-2 flex items-center space-x-1.5 overflow-hidden">
                    <button
                      onClick={(e) => handleOpenDossier(msg.sender, e)}
                      className="font-bold text-cyan-400 hover:text-cyan-300 hover:underline truncate flex items-center space-x-1 text-left"
                  ...
                  {/* Message Body */}
                  <div className="col-span-4 md:col-span-7 break-words pr-2 font-sans select-text leading-relaxed text-xs">
                    {msg.message}
                  </div>
                  ...
                  <div className="col-span-1 flex items-center justify-end space-x-1.5">
                    ...
                    >
                      {msg.isFlagged ? (
                        <BookmarkCheck className="w-4 h-4 fill-rose-500/20" />
                      ) : (
