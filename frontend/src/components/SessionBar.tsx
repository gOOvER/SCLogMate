    ...
    <div className="flex items-center justify-between px-5 py-2 border-b border-cyan-950/50 bg-[#030814]/90 z-15 shrink-0 gap-3 text-xs">
      ...
      <div className="flex items-center gap-2.5 flex-1 min-w-0">
        ...
        <span className="text-slate-300 font-mono text-xs truncate">
          ...
          <span className="text-slate-500 hidden sm:inline">{t('sessionBar.currentSessionDesc')}</span>
        </span>
      </div>
      ...
      <div className="flex items-center gap-2.5 shrink-0">
        ...
        <div
          ...
          title={t('sessionBar.timeSpanTitle')}
        >
        ...
        <button
          ...
          title={isHudCollapsed ? t('sessionBar.hudExpandTooltip') : t('sessionBar.hudCollapseTooltip')}
        >
          {isHudCollapsed ? (
            <>
              <ChevronDown className="w-3.5 h-3.5 text-cyan-400" />
              <span>{t('sessionBar.showHud')}</span>
            </>
          ) : (
            <>
              <ChevronUp className="w-3.5 h-3.5" />
              <span className="hidden sm:inline">{t('sessionBar.minimizeHud')}</span>
            </>
          )}
        </button>
      </div>
