    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/75 backdrop-blur-sm animate-fade-in">
      ...
      >
        ...
        <div className="px-5 py-3.5 border-b border-[#142A45] flex items-center justify-between bg-gradient-to-r from-[#071322] to-[#0A1A2F]">
          ...
          <button
            onClick={onClose}
            className="p-1.5 rounded-lg text-slate-400 hover:text-slate-100 hover:bg-slate-800/70 transition cursor-pointer"
            title="Schließen"
          >
            <X className="w-5 h-5" />
          </button>
            ...
            {components.length === 0 ? (
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                {components.map((c, idx) => (
                  <div
                    key={`${c.slotType}-${idx}`}
                    className={`p-2.5 rounded-lg border flex items-center gap-2.5 transition ${getSlotColor(c.slotType)}`}
                  >
