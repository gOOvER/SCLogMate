...
### Added
- **Redesigned Star Citizen `user.cfg` Suite & Dedicated Backup Vault (`ToolsView.tsx`)**:
  - Rebuilt `user.cfg` management in `ToolsView.tsx` into a spacious, uncluttered layout with dedicated navigation tabs and persistent sub-tab pills:
    - **`user.cfg Live-Editor`**: Full-width code editor with line counts, characters count, instant copy, reload, and safe saving with automatic multi-location backup.
    - **`Grafik- & Performance-Tuning`**: Multi-card visual studio with live-merging controls organized into 7 clear categories: CPU & Thread Management (`r_multithreaded = 1`), Monitor, FPS & Synchronization (`sys_maxFps = 160`, `r_VSync = 0`, `r_enable_full_gpu_sync = 0`, `r_FullscreenWindow = 1`, `r_BorderlessWindow = 1`), VRAM & Streaming (`r_TexturesStreamPoolSize = 8192`, `e_StreamCgfPoolSize = 4096`, `r_TexturesStreaming = 1`), Native HDR Display Output (`r_HDRDisplayOutput = 1`, `r_HDRDisplayMaxNits = 1000`, `r_HDRDisplayRefWhite = 200`, `r_HDRDisplayDeviceLimits = 1`), Shader & Cache Stutter Prevention (`r_shadersasyncactivation = 1`, `r_GsmCache = 1`), Visual Fidelity & Details (`r_MotionBlur = 0`, `r_ssdo = 1/2`, `e_ParticlesQuality = 3`, `r_DetailDistance = 22`), and Interface & Localization (`pl_pit.forceSoftwareCursor = 0`, `r_DisplayInfo`, `g_language`, `g_languageAudio`).
    - **`Befehls-Lexikon (Stand 2026)`**: Comprehensive interactive cheat sheet and documentation of all 2026 Star Citizen engine cvars with real-time search, value range descriptions, and 1-click value adoption.
    - **`Backup-Tresor & 1-Klick Rollback`**: Dedicated backup management view featuring manual snapshot creation with custom notes, backup history list with formatted timestamps, file sizes, and storage badges (Local / Cloud), and 1-click rollback restoration.
  - Added dedicated preset for **AMD Ryzen 7 5800X3D & NVIDIA RTX 5070** with options to either merge non-destructively or deploy the pure template with extensive comments.
  - Enforced `Con_Restricted = 0` placement at the very top of `user.cfg` during merges to guarantee proper engine loading.
...
- **In-Game Chat-Protokoll & Background OCR (`Sidebar.tsx`, `Settings.cs`, `PhotinoBridge.cs`, `SettingsView.tsx`, `ROADMAP.md`)**:
...
- **In-Game Chat OCR Multi-Line Message Parsing, Chronicle Display & Compact Layout (`OcrEngineService.cs`, `ChatParser.cs`, `ChatOcrScanner.cs`, `Settings.cs`, `Database.cs`, `PhotinoBridge.cs`, `ChatLogView.tsx`)**:
  ...
  - Added fuzzy Levenshtein similarity matching (`ChatParser.IsDuplicateMessage`) in `ChatOcrScanner.cs` and `Database.cs`, eliminating duplicate log entries caused by static chat lines lingering on the screen across 3.5s scan ticks with OCR character jitter (such as `raffenerie` vs `raffenprie`, punctuation variations, or slight sender discrepancies like `goovER` vs `goovEk`).
  - Added glued channel prefix extraction (`ExtractGluedChannel`) in `ChatParser.cs` to cleanly disentangle channel tags fused directly to pilot handles when OCR omits separating spaces (e.g. `tGLOBALKAnthonyBenson` -> Channel: `Global`, Sender: `AnthonyBenson`; `BCXRAUTZJAnthonyBenson` -> Channel: `SC Krautz`, Sender: `AnthonyBenson`).
  - Added in-memory recent message history clearing (`ClearRecentHistory`) in `ChatOcrScanner.cs` tied to the UI chat clear action.
