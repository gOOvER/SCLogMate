...
{
    public static string GetRegionalHost(string? shard, string? regionCode = null)
    {
        var s = (shard ?? "").ToLowerInvariant().Trim();
        var r = (regionCode ?? "").ToLowerInvariant().Trim();

        // 1. Australien / Ozeanien (Muss VOR "us" geprüft werden, da "aus" die Teilzeichenkette "us" enthält!)
        if (r == "aus" || s.Contains("aus") || s.Contains("apse") || s.Contains("oce") || s.Contains("syd") || s.Contains("ap-south"))
            return "ec2.ap-southeast-2.amazonaws.com"; // Australien (Sydney)

        // 2. Asien / Pazifik
        if (r == "asia" || s.Contains("asia") || s.Contains("apne") || s.Contains("ape") || s.Contains("aps") || s.Contains("jp") || s.Contains("sg") || s.Contains("tyo") || s.Contains("tokyo") || s.Contains("hkg"))
        ...
        if (r == "us" || s.Contains("usw") || s.Contains("us-west") || s.Contains("oregon") || s.Contains("-or-") || s.Contains("na") || s.Contains("us"))
        ...
        if (r == "eu" || s.Contains("euw") || s.Contains("eun") || s.Contains("lon") || s.Contains("irl") || s.Contains("dublin") || s.Contains("eu"))
            return "ec2.eu-west-1.amazonaws.com"; // Irland / Europa

        return "ec2.eu-west-1.amazonaws.com";
    }

    public static async Task<long?> MeasureLatencyAsync(string? shard, string? regionCode = null, int timeoutMs = 1500, CancellationToken ct = default)
    {
        var host = GetRegionalHost(shard, regionCode);
        try
        {
